# Desktop Defender — guide scripts
...
```bash
python scripts/extract_loot.py "/path/to/Desktop Defender" --generate-guide
```
...
**How extraction works:** The script loads `data.unity3d` and walks `MonoBehaviour` assets using field-name conventions — `lootName` for items, `craftMaterial1`/`craftMaterial2` for recipes, `allItems` for pools, and `entries` for drop tables. It does not depend on a specific assembly name.
...
Output goes to `diagrams/{item-slug}-diagram.png`. Use `--only "Boson"` to export a single diagram.
